...
$select_transection = mysqli_query($con, "SELECT * FROM `transactions` WHERE `user_id` = '$user_id'")
...
// $select = mysqli_query($con, "SELECT * FROM `keepers` WHERE `kepper_ids` = '$user_id'");
// //echo "SELECT * FROM `keepers` WHERE `kepper_ids` = '$user_id'";
// $userIds = array();
// while($keepers = mysqli_fetch_assoc($select)){
//      //$user_ids = $keeprs['user_id'];
//      $userIds[] = $keepers['user_id'];
//     //  mysqli_query($con, "UPDATE `users` SET `membership`='1' WHERE `ID` = '$user_ids'");
//     //  echo "UPDATE `users` SET `membership`='1' WHERE `ID` = '$user_ids'";
// }
// die();
