...
if(isset($_POST['username']) && !empty($_POST['username'])){
    ...
    if($check_username_response->num_rows > 0){
        ...
        if($zip->open( $_SERVER['DOCUMENT_ROOT'] . "/download/" . $zipname, ZIPARCHIVE::CREATE )){
            ...
            $type = 3;
            $mementose_public_pictures = $connect->prepare("SELECT * FROM `mementose_pictures` WHERE `user_id` = ? AND `type` = ?");
            $mementose_public_pictures->bind_param("ss", $check_username_fetch['ID'], $type);
